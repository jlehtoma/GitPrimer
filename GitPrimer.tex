...
\ei
\item Use {\tt .gitignore} file to indicate files to be ignored
\ei
...
Use {\tt \color{hilight} git rm} instead of just {\tt rm}

Use {\tt \color{hilight} git mv} instead of just {\tt mv}
...
\item {\tt git add .} {\footnotesize \color{lolight} (or name them individually)}
\item {\tt git status} {\footnotesize \color{lolight} (did you miss any?)}
\item {\tt git commit}
\ei
...
\item Copy contents of {\color{hilight} \verb|~/.ssh/id_rsa.pub|}
\ei
...
\item Add SSH key
\ei
\item Similar thing at BitBuckeet
\ei
\end{frame}
...
\item Back at the command line:
  \bi
  \item {\tt git remote add origin git@github.com:username/repo}
